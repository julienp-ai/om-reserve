...
export default async function MatchDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <Link href="/dashboard/matches" className={buttonVariants({ variant: 'ghost', size: 'sm' }) + ' -ml-2 gap-1.5 text-muted-foreground hover:text-foreground'}>
      ...
      <div className="rounded-2xl border border-border bg-card px-6 py-5">
        <h2 className="font-semibold text-foreground mb-4">
          {reservation ? 'Ma réservation' : 'Demander des places'}
        </h2>
            ...
            <div className="flex items-center justify-between rounded-lg bg-muted/50 px-4 py-3">
              <div>
                <p className="text-sm font-medium text-foreground">
                  {reservation.seats_requested} place{reservation.seats_requested > 1 ? 's' : ''} demandée{reservation.seats_requested > 1 ? 's' : ''}
                </p>
                {reservation.admin_note && (
                  <p className="text-xs text-muted-foreground mt-1">Note : {reservation.admin_note}</p>
                )}
              </div>
              <ReservationStatusBadge status={reservation.status} />
            </div>
            {reservation.status === 'pending' && (
              <ReservationForm matchId={id} userId={user.id} existingReservation={reservation as Reservation} mode="cancel" />
            )}
          </div>
        ) : isFull ? (
          <p className="text-muted-foreground text-sm">
            Ce match est complet. Aucune place n&apos;est disponible.
          </p>
        ) : (
          <ReservationForm matchId={id} userId={user.id} mode="create" />
        )}
      </div>
    </div>
  )
